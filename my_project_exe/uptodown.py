...
# Navigate to the Uptodown website
    driver.get('https://en.uptodown.com/')
    time.sleep(3)

# XPaths for the elements
    appsticklogoXPath = "//div[@id='menu-button-id']"
    sidebarpolpular = "//div[@id='search-button']"
...
# If all conditions are satisfied, write 'open', else write 'block'
    if isAppStickLogoPresent and isloginXPath and isdownloadXPath:
        ...
        sheet.append(["uptodown", "https://en.uptodown.com/", "open"])
    else:
        print("One or more elements are not present, adding 'block' status to Excel file")
        sheet.append(["uptodown", "https://en.uptodown.com/", "block"])
