...
# Navigate to the website
    driver.get('https://softradar.com/')
    time.sleep(3)

# XPaths for the elements
    appsticklogoXPath = "//a[@id='brand-button']"
    sidebarpolpular = "//span[contains(text(),'ANDROID')]"
    sidebarprogramming = "//span[contains(text(),'MAC')]"
...
# If all conditions are satisfied, write 'open', else write 'block'
    if isAppStickLogoPresent and isloginXPath and isdownloadXPath:
        ...
        sheet.append(["softradar", "https://softradar.com/", "open"])
    else:
        print("One or more elements are not present, adding 'block' status to Excel file")
        sheet.append(["softradar", "https://softradar.com/", "block"])
