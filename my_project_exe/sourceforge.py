...
# Navigate to the SourceForge website
    driver.get('https://sourceforge.net/')
    time.sleep(3)

# XPaths for the elements
    appsticklogoXPath = "//nav[@class='links']//a[text() = 'Create']"
    sidebarpolpular = "//nav[@class='links']//a[text() = 'Join']"
    sidebarprogramming = "//nav[@class='links']//a[text() = 'Login']"
...
# If all conditions are satisfied, write 'open', else write 'block'
    if isAppStickLogoPresent and isloginXPath and isdownloadXPath:
        ...
        sheet.append(["sourceforge", "https://sourceforge.net/", "open"])
    else:
        print("One or more elements are not present, adding 'block' status to Excel file")
        sheet.append(["sourceforge", "https://sourceforge.net/", "block"])
